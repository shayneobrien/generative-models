...
class Trainer:
    ...
    def train(self, model, num_epochs, G_lr = 2e-4, D_lr = 2e-4, G_init = 5, D_steps = 1):
        ...
        Inputs:
            ...
            G_init: int, amount of steps to train G before beginning to jointly train G and D (default 0)
    ...
    def train_D(self, model, images):
        ...
        """    
        # Labels for binarized MNIST (X is real (ones), G_output is generated(zeros))        
    ...
    def train_G(self, model, images):
        ...
        G_loss = F.binary_cross_entropy((1-DG_score).squeeze(), G_labels)
        return -G_loss
...
model = trainer.train(model = model, num_epochs = 100, G_lr = 2e-4, D_lr = 2e-4, G_init = 5, D_steps = 1)
